...
import {CORE_CONTRACT_ADDRESS, TOKENS_CONTRACT_ADDRESS, START_BLOCK} from '@darkforest_eth/contracts'
...
async function main(){
    ...
    const abisPath = path.join(process.cwd(), 'node_modules/@darkforest_eth/contracts/abis');
    const yaml = (await fs.readFile(path.join(subgraphPath, 'subgraph.template.yaml')))
      ...
      .replace(/{{{TOKENS_CONTRACT_ADDRESS}}}/g, TOKENS_CONTRACT_ADDRESS)
      ...
      .replace(/{{{TOKENS_ABI_PATH}}}/g, path.join(abisPath, 'DarkForestTokens_stripped.json'))
